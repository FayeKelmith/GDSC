...
llm = ChatOpenAI(api_key=openai_key, temperature=0.8)
SYSTEM_TEMPLATE = """
...
chat_room, load_link = st.tabs("Chat Room", "Load Link")
...
with load_link:
    ...
    if web_link:
        st.write(f"Getting data from site to db")
        loader = WebBaseLoader(web_link)
    if web_link:
        st.info("What would like to know from this link: ")
        data = loader.load()
...
if prompt := st.chat_input():
    ...
    st.info(f"Scraping from {web_link}")
    ...
    lambda x: len(x.get("history", [])) == 1,
        ...
        ), question_answering_prompt | llm | StrOutputParser() | retriever,).with_config(run_name="chat_retriever_chain")
                
    chain_with_history = RunnableWithMessageHistory( query_transforming_retriever_chain, lambda session_id: history,input_messages_key="question", history_messages_key="history",)
